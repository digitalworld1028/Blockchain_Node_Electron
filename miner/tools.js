...
module.exports = {
  ...
  createRandomNonce: () => {
    return sha256(Math.random()*1000000+new Date().toString());
  },
  ...
  getLastMADLedger(cb) {
    axios.get('http://192.46.216.136/getLastEntries').then(resp => {
      cb({state: true, lastMADLedger: resp.data});
    }).catch((error) => {
      console.log(error);
      cb({state: false});
    });
  ...
  createBlock(transactionsForMining, prevhash, id, nonce, timestamp = null) {
    const transactions = transactionsForMining;
    if(timestamp == null) timestamp = Date.now() / 1000;
    const hash = sha256(JSON.stringify({id: id, prevhash: prevhash, nonce: nonce, transactions: transactions, timestamp: timestamp}));
    return {id: id, prevhash: prevhash, nonce: nonce, transactions: transactions, timestamp: timestamp, hash: hash};
  },
